import numpy as np
from basics.generics import DBZ, DBZArray, get_margin
from basics.indices import index_block
...
def generate_equilateral_triangle(orientation = np.pi/2, base_width = 10):
    p0 = [-base_width/2, 0]
    p1 = [base_width/2, 0]
    base_line = generate_perpendicular_line(p0, p1)
    height = base_width * .866 # steps
    point = (0, round(height))
    lines = []
    for step in range(1, round(height)):
        lines.append(generate_perpendicular_line([p0[0] + step, ]))
    pass
                     
def generate_triangle(p0, p1, p2, interior = True):
    l1 = generate_line(p0, p1)
    l2 = generate_line(p0, p2)
    l3 = generate_line(p1, p2)
    bounds = np.concatenate([l1,l2,l3], axis = 1)
    ...
    full = np.empty(shape = (2,0), dtype = int)

    #TODO: find max size of any row and make numpy-only algorithm to generate row by row subarrays
    for row in range(x_min, x_max + 1):
        y_rng = bounds[1, bounds[0] == row]
        y_min = y_rng.min()
        y_max = y_rng.max()
        ys = np.arange(y_min, y_max + 1)
        new = np.empty(shape = (2,ys.size), dtype = int)
        new[0] = row
        new[1] = ys
        full = np.concatenate([full, new], axis = 1)
...
def generate_arrow(p1, p2, line_thick, head_thick):
    ...
    return np.concatenate((line, head), axis = 1)
